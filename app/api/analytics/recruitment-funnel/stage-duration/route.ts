// Phase 8.2R: GET /api/analytics/recruitment-funnel/stage-duration
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const stageDurations = result.stages
      .filter(s => s.avgDurationDays !== null)
      .map(s => ({ stageKey: s.stageKey, label: s.label, avgDurationDays: s.avgDurationDays, count: s.count }));
    return Response.json({ success: true, data: { stageDurations, dataQualityWarnings: result.dataQualityWarnings } });
  } catch {
    return Response.json({ success: false, error: "阶段停留时长加载失败" }, { status: 500 });
  }
}
