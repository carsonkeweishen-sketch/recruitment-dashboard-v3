// Phase 8.2R: GET /api/analytics/recruitment-funnel/stages
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ success: true, data: { stages: result.stages, dataQualityWarnings: result.dataQualityWarnings } });
  } catch {
    return Response.json({ success: false, error: "阶段数据加载失败" }, { status: 500 });
  }
}
