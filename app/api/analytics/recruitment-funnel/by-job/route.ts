// Phase 8.2R: GET /api/analytics/recruitment-funnel/by-job
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ success: true, data: { byJob: result.byJob } });
  } catch {
    return Response.json({ success: false, error: "岗位对比数据加载失败" }, { status: 500 });
  }
}
