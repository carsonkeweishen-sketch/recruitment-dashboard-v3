...
4. [Layer 3：Autonomous Intelligence（自主智能 / 有限自动决策）](#4-layer-3autonomous-intelligence自主智能--有限自动决策)
5. [AI 数据契约](#5-ai-数据契约)
...
## 4. Layer 3：Autonomous Intelligence（自主智能 / 有限自动决策）

### 4.1 定义

AI 在**严格约束条件**下自主执行低风险操作。所有自动操作必须可追溯、可撤销。
...
| # | 能力 | 自动操作 | 安全约束 | 当前状态 |
|---|------|---------|---------|---------|
| L3-1 | 自动生成周报初稿 | 创建 ReportSnapshot (status=draft) | 仅 draft，需人 confirm 后才发布 | 🔲 Phase 13+ |
| L3-2 | 自动推荐知识库模板 | 在创建岗位时推荐匹配模板 | 仅推荐，不自动应用 | 🔲 Phase 14+ |
| L3-3 | 自动标记低质量反馈 | 设置 feedbackQualityScore 和 qualityLevel | 需人复核，标记可修改 | 🔲 Phase 14+ |

### 4.4 安全约束（不可逾越的红线）

```
┌─────────────────────────────────────────────────────────┐
│  Layer 3 安全红线（违反任何一条，禁止上线）                   │
│                                                         │
│  ❌ 绝不自动推进/淘汰候选人                                  │
│  ❌ 绝不自动发送外部通知（邮件/短信/飞书）                      │
│  ❌ 绝不自动修改岗位画像（只生成校准建议）                      │
...
│  ❌ 绝不自动修改权限或角色                                   │
│                                                         │
│  ✅ 所有自动操作必须有 AiAnalysisLog 记录                    │
│  ✅ 所有自动操作必须可撤销（通过 ActivityLog 回滚）             │
│  ✅ 所有自动操作结果必须对人可见                              │
│  ✅ 人可随时关闭任意 Layer 3 能力                            │
└─────────────────────────────────────────────────────────┘
```
...
前置条件：
- Layer 2 在至少 2 个能力上验证成熟（置信度 > 85%）
- AiAnalysisLog 累计 > 1000 条确认记录
- 人工确认率 > 70%（说明 AI 建议质量可靠）
